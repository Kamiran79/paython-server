...
class HandleRequests(BaseHTTPRequestHandler):
    ...
    def do_GET1(self):
        ...
        if resource == "animals1":
            if id is not None:
                response = f"{get_single_animal(id)}"
            else:
                response = f"{get_all_animals()}"
        elif resource == "locations1":
            if id is not None:
    ...
    def do_GET(self):
        ...
        def do_POST(self):
            ...
            # Parse the URL
            (resource, _id) = self.parse_url(self.path)
    ...
    def do_DELETE(self):
        ...
        # Encode the new animal or item selected to be delete and send in response
